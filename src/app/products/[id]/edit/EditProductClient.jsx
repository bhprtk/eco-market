...
export default function EditProductClient({ product }) {
	...
	const handleDelete = async () => {
		console.log("Product received in EditProductClient:", product);

		if (!user) return alert("Login required");
		if (!product?.id) return alert("Invalid product ID");
			...
			<form onSubmit={handleSubmit} className="space-y-4">
				...
				<div className="flex gap-4">
					...
					<button
						...
						className="border border-red-600 text-red-600 px-6 py-2 rounded-md hover:bg-red-600 hover:text-white transition cursor-pointer"
					>
						Delete Product
					</button>
				</div>
